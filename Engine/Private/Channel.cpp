...
void CChannel::Compute_TransformationMatrix(_double PlayTime)
{
	while (m_iCurrentKeyFrame + 1 < m_KeyFrames.size())
	{
		if (PlayTime > m_KeyFrames[m_iCurrentKeyFrame + 1]->Time) {
			...
			m_iLastKeyFrame = m_iCurrentKeyFrame;
			break;
		}
	}
...
void CChannel::Change_Compute_TransformationMatrix(_double PlayTime, KEYFRAME PreKey)
{

	m_iCurrentKeyFrame = 0;
	_vector		vScale, vRotation, vTranslation;

	_double		Ratio = PlayTime;

	_vector		vSourScale = XMLoadFloat3(&PreKey.vScale);
	_vector		vSourRotation = XMLoadFloat4(&PreKey.vRotation);
	_vector		vSourTranslation = XMLoadFloat3(&PreKey.vTranslation);
