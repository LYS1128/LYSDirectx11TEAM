...
HRESULT CCollider::NativeConstruct_Prototype(TYPE eType)
{
	...
	//릴리즈 할때 eType 관련 부부 ㄴ빼고 디버그 안에 돌리면 릴리즈 잘된다고... ㅇㅋㅇㅋ...
...
_bool CCollider::Collsion(CCollider * pTargetCollider)
{
	if (TYPE_AABB == pTargetCollider->m_eType)
		m_isCollision = m_pAABBInWorld->Intersects(*pTargetCollider->m_pAABBInWorld);

	if (TYPE_OBB == pTargetCollider->m_eType)
		m_isCollision = m_pOBBInWorld->Intersects(*pTargetCollider->m_pOBBInWorld);
...
{
#pragma region How to do Method
	/*if (TYPE_AABB == pTargetCollider->m_eType)
	{
...
#pragma endregion
	...
	if (TYPE_AABB == pTargetCollider->m_eType)
		m_isCollision = m_pOBBInWorld->Intersects(*pTargetCollider->m_pAABBInWorld);

	if (TYPE_OBB == pTargetCollider->m_eType)
		m_isCollision = m_pOBBInWorld->Intersects(*pTargetCollider->m_pOBBInWorld);

	if (TYPE_SPHERE == pTargetCollider->m_eType)
		m_isCollision = m_pOBBInWorld->Intersects(*pTargetCollider->m_pSPHEREInWorld);

	return m_isCollision;
}

_bool CCollider::Collsion_Sphere(CCollider * pTargetCollider)
{
	if (TYPE_AABB == pTargetCollider->m_eType)
		m_isCollision = m_pSPHEREInWorld->Intersects(*pTargetCollider->m_pAABBInWorld);

	if (TYPE_OBB == pTargetCollider->m_eType)
		m_isCollision = m_pSPHEREInWorld->Intersects(*pTargetCollider->m_pOBBInWorld);
...
_vector CCollider::InterSectsRay(_vector vRayPos, _vector vRayLook)
{
	_float Dist = 1.0f;
	_bool flag = false;
	...
	case Engine::CCollider::TYPE_AABB:
		flag = m_pAABBInWorld->Intersects(vRayPos, vRayLook, Dist);
		break;
	case Engine::CCollider::TYPE_OBB:
		flag = m_pOBBInWorld->Intersects(vRayPos, vRayLook, Dist);
		break;
	case Engine::CCollider::TYPE_SPHERE:
		flag = m_pSPHEREInWorld->Intersects(vRayPos, vRayLook, Dist);
		break;
	default:
		break;
	}
	if (flag == true) {
		vRayLook = XMVector3Normalize(vRayLook) * Dist;
		vRayPos += vRayLook;
		return vRayPos;
	}
