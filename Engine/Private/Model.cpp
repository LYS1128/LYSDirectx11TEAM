...
HRESULT CModel::NativeConstruct_Prototype(TYPE eType, const char * pModelFilePath, const char * pModelFileName, _fmatrix PivotMatrix)
{
	...
	if (TYPE_NONANIM == m_eType)
		...
		iFlag = aiProcess_ConvertToLeftHanded | aiProcess_CalcTangentSpace | aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_ImproveCacheLocality;
	...
	XMStoreFloat4x4(&m_PivotMatrix, PivotMatrix);
		...
		if (nullptr == m_pScene) {
			MSGBOX("Model Component -> m_pScene = nullptr !!");
			return E_FAIL;
		}

		if (FAILED(Ready_HierarchyNodes(m_pScene->mRootNode))) {
			MSGBOX("Failed Ready_HierarchyNodes");
			return E_FAIL;
		}
		...
		if (FAILED(Ready_MeshContainers())) {
			MSGBOX("Failed Ready_MeshContainer");
			return E_FAIL;
		}

		if (FAILED(Ready_Materials(pModelFilePath))) {
			MSGBOX("Failed Ready_Materials");
			return E_FAIL;
		}
		...
		if (FAILED(Ready_Animations())) {
			MSGBOX("Failed Ready_Animations");
			return E_FAIL;
		}
...
_bool CModel::Update(_double TimeDelta, _bool isLinear)
{
	if (m_iCurrentAnimIndex >= m_iNumAnimations)
		return false;

	if (isLinear == true && (m_iCurrentAnimIndex != m_iPreAnimIndex || m_bChangeEnd == false)) {
		...
		if (m_Animations[m_iCurrentAnimIndex]->Change_Animation(m_PreKey, TimeDelta)) {
			m_Animations[m_iPreAnimIndex]->Set_CurrentKeyFrame_Zero();
		...
		if (isLinear == false && m_iCurrentAnimIndex != m_iPreAnimIndex) {
			m_Animations[m_iPreAnimIndex]->Set_KeyFrameZero();
			m_Animations[m_iPreAnimIndex]->Set_TimeAcc_Zero();
			m_Animations[m_iPreAnimIndex]->Set_KeyFrameEnd(false);
			m_iPreAnimIndex = m_iCurrentAnimIndex;
...
HRESULT CModel::Ready_Materials(const char* pModelFilePath)
{
	...
	m_ModelDatadesc.NumMatData = m_vMatData.size();
	...
	for (int i = 0; i < m_iNumMaterials; i++) {
		...
		for (int j = k; j < m_vMatData.size(); j++) {
			if (m_vMatData[j].MatIndex == i) {
				_tchar		szFullPath[MAX_PATH] = TEXT("");
				MultiByteToWideChar(CP_ACP, 0, m_vMatData[j].FilePath, strlen(m_vMatData[j].FilePath), szFullPath, MAX_PATH);
...
HRESULT CModel::Ready_HierarchyNodes(aiNode* pNode, CHierarchyNode* pParent, _uint iDepth)
{
	...
	m_iNumNodes = m_HierarchyNodes.size();
