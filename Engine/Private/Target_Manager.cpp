...
ID3D11ShaderResourceView * CTarget_Manager::Get_LightDepthSRV() const
{
	return m_pLightDepth->Get_SRV();
}
...
HRESULT CTarget_Manager::Add_LightDepth(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CLightDepth*		pLightDepth = CLightDepth::Create(pDevice, pDeviceContext);
	if (nullptr == pLightDepth)
		return E_FAIL;

	m_pLightDepth = pLightDepth;
...
HRESULT CTarget_Manager::Begin_LightDepth(ID3D11DeviceContext * pDeviceContext)
{
	pDeviceContext->OMGetRenderTargets(1, &m_pBackBufferRTV, &m_pDepthStencilView);
	m_pLightDepth->BindDsvAndSetNullRenderTarget(pDeviceContext);
	return S_OK;
}

HRESULT CTarget_Manager::End_LightDepth(ID3D11DeviceContext * pDeviceContext)
{
	...
	pDeviceContext->RSSetViewports(1, &ViewPortDesc);
	return S_OK;
}
...
HRESULT CTarget_Manager::Ready_LightDebugDesc(_float fTopX, _float fTopY, _float fWidth, _float fHeight)
{
	return m_pLightDepth->Ready_DebugDesc(fTopX, fTopY, fWidth, fHeight);
}
HRESULT CTarget_Manager::Render_LightDebug()
{

	if (FAILED(m_pLightDepth->Bind_WorldMatrix(m_pShader)))
		return E_FAIL;
	if (FAILED(m_pShader->Set_RawValue("g_ViewMatrix", &m_ViewMatrix, sizeof(_float4x4))))
		return E_FAIL;
	if (FAILED(m_pShader->Set_RawValue("g_ProjMatrix", &m_ProjMatrix, sizeof(_float4x4))))
