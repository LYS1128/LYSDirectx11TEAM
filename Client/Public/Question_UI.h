...
class CQuestion_UI final : public CUI
{
public:
	enum IDEA_INDEX {
...
public:
	...
	{		
		...
		_float			fSizeX;
		_float			fSizeY;
		IDEA_INDEX		eQuestion_IdeaIndex;
	}UI;

private:
	explicit CQuestion_UI(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut);
	explicit CQuestion_UI(const CQuestion_UI& rhs);
	virtual ~CQuestion_UI() = default;
...
private:
	void				Question_Image();
public:
	static CQuestion_UI* Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut);
