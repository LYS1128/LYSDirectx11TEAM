#include "stdafx.h"
#include "..\Public\UI_F_SubAbility.h"
#include "Equipment_Manager.h"
#include "Client_Level_Manager.h"
#include "UI_SubAbility_Coltime.h"
#include "Scene_Manager.h"

CUI_F_SubAbility::CUI_F_SubAbility(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CUI(pDeviceOut, pDeviceContextOut)
{
}

CUI_F_SubAbility::CUI_F_SubAbility(const CUI_F_SubAbility & rhs)
	: CUI(rhs)
{
}

HRESULT CUI_F_SubAbility::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype())) {
		MSG_BOX(L"Failed To CUI_F_SubAbility : NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CUI_F_SubAbility::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(pArg))) {
		MSG_BOX(L"Failed To CUI_F_SubAbility : NativeConstruct, __super::NativeConstruct");
		return E_FAIL;
	}
	if (FAILED(SetUp_Components())) {
		MSG_BOX(L"Failed To CUI_F_SubAbility : NativeConstruct, SetUp_Components");
		return E_FAIL;
	}
...
void CUI_F_SubAbility::Tick(_double TimeDelta)
{
	...
	_uint i = CEquipment_Manager::Get_Instance()->Get_SkillInfo().eIndex;
...
HRESULT CUI_F_SubAbility::SetUp_Components()
{
	/* For.Com_Renderer */
...
CUI_F_SubAbility * CUI_F_SubAbility::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CUI_F_SubAbility*	pInstance = new CUI_F_SubAbility(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CUI_F_SubAbility"));
...
CGameObject * CUI_F_SubAbility::Clone(void * pArg)
{
	CUI_F_SubAbility*	pInstance = new CUI_F_SubAbility(*this);
