#include "stdafx.h"
#include "..\Public\Goods_Text_Num.h"
#include "Player_Manager.h"
#include "Bar_Waterdrop_Text_Number.h"
#include "Scene_Manager.h"
#include "Client_Level_Manager.h"

CGoods_Text_Num::CGoods_Text_Num(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CUI(pDeviceOut, pDeviceContextOut)
{
}

CGoods_Text_Num::CGoods_Text_Num(const CGoods_Text_Num & rhs)
	: CUI(rhs)
{
}

HRESULT CGoods_Text_Num::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype())) {
		MSG_BOX(L"Failed To CGoods_Text_Num : NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CGoods_Text_Num::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(pArg))) {
		MSG_BOX(L"Failed To CGoods_Text_Num : NativeConstruct, __super::NativeConstruct");
		return E_FAIL;
	}
	
	if (FAILED(SetUp_Components())) {
		MSG_BOX(L"Failed To CGoods_Text_Num : NativeConstruct, SetUp_Components");
		return E_FAIL;
	}
...
void CGoods_Text_Num::Tick(_double TimeDelta)
{
	...
	default:
		...
		CGameObject*	pLeaf = pGameInstance->Get_GameObject(LEVEL_STATIC, TEXT("Layer_Goods_Leaf_Num"), 0);//한번만 가져와서 실시간으로 렌더
		dynamic_cast<CGoods_Text_Num*>(pLeaf)->Set_Num(CPlayer_Manager::Get_Instance()->Get_Leaf_UI_Num());


	RELEASE_INSTANCE(CGameInstance);
}

void CGoods_Text_Num::LateTick(_double TimeDelta)
{
...
HRESULT CGoods_Text_Num::Render()
{
...
HRESULT CGoods_Text_Num::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Texture */
		...
		if (FAILED(__super::SetUp_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Blue_Num"), (CComponent**)&m_pTextureCom)))
			return E_FAIL;
...
CGoods_Text_Num * CGoods_Text_Num::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CGoods_Text_Num*	pInstance = new CGoods_Text_Num(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CGoods_Text_Num"));
...
CGameObject * CGoods_Text_Num::Clone(void * pArg)
{
	CGoods_Text_Num*	pInstance = new CGoods_Text_Num(*this);
