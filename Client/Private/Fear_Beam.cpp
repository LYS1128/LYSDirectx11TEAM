#include "stdafx.h"
#include "..\Public\Fear_Beam.h"
#include "GameInstance.h"
#include "Time_Manager.h"
#include "Stage_Manager.h"
#include "Monster_Manager.h"

CFear_Beam::CFear_Beam(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{
}

CFear_Beam::CFear_Beam(const CFear_Beam & rhs)
	: CGameObject(rhs)
{
}

HRESULT CFear_Beam::NativeConstruct_Prototype()
{
...
HRESULT CFear_Beam::NativeConstruct(void * pArg)
{
...
void CFear_Beam::Tick(_double TimeDelta)
{
	...
	m_fDist = CMonster_Manager::Get_Instance()->Get_Distance();
	m_pTargetMatrix = CMonster_Manager::Get_Instance()->Get_BeamMatrix();
...
void CFear_Beam::LateTick(_double TimeDelta)
{
	_double TimeRatio = CTime_Manager::Get_Instance()->Get_MonsterRatio();
	_double mTimeDelta = TimeDelta * TimeRatio;
	...
	if (FAILED(pGameInstance->Add_GameObjectToLayer(LEVEL_STATIC, TEXT("Layer_Fear_Beam_Mouse"), TEXT("Prototype_GameObject_Fear_Beam_Mouse"), &m_pTargetMatrix))) {
		MSG_BOX(L"Failed To CLevel_GamePlay : Scroll_UI,pGameInstance->Add_GameObjectToLayer, Scroll_UI");
	}
...
HRESULT CFear_Beam::SetUp_Components()
{
	/* For.Com_Renderer */
...
CFear_Beam * CFear_Beam::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CFear_Beam*	pInstance = new CFear_Beam(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CFear_Beam"));
...
CGameObject * CFear_Beam::Clone(void * pArg)
{
	CFear_Beam*	pInstance = new CFear_Beam(*this);
