#include "stdafx.h"
#include "..\Public\UI_StageEnd_RS_Text.h"
#include "Equipment_Manager.h"
#include "Client_Level_Manager.h"
#include "Percent_Tex.h"
#include "Endgame_Manager.h"

CUI_StageEnd_RS_Text::CUI_StageEnd_RS_Text(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CUI(pDeviceOut, pDeviceContextOut)
{
}

CUI_StageEnd_RS_Text::CUI_StageEnd_RS_Text(const CUI_StageEnd_RS_Text & rhs)
	: CUI(rhs)
{
}

HRESULT CUI_StageEnd_RS_Text::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype())) {
		MSG_BOX(L"Failed To CUI_StageEnd_RS_Text : NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CUI_StageEnd_RS_Text::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(pArg))) {
		MSG_BOX(L"Failed To CUI_StageEnd_RS_Text : NativeConstruct, __super::NativeConstruct");
		return E_FAIL;
	}
	if (FAILED(SetUp_Components())) {
		MSG_BOX(L"Failed To CUI_StageEnd_RS_Text : NativeConstruct, SetUp_Components");
		return E_FAIL;
	}
	...
	if (m_tNumberInfo.iCipher == 1 && *m_tNumberInfo.bCheck == true)
	{
		if (FAILED(pGameInstance->Add_GameObjectToLayer(LEVEL_STATIC, TEXT("Layer_Num"), TEXT("Prototype_GameObject_Tex_Morae"), &tPercentInfo))) {
...
void CUI_StageEnd_RS_Text::Tick(_double TimeDelta)
{
...
void CUI_StageEnd_RS_Text::LateTick(_double TimeDelta)
{
...
HRESULT CUI_StageEnd_RS_Text::Render()
{
	if (m_bRenderTrue)
	{
...
HRESULT CUI_StageEnd_RS_Text::SetUp_Components()
{
	/* For.Com_Renderer */
...
CUI_StageEnd_RS_Text * CUI_StageEnd_RS_Text::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CUI_StageEnd_RS_Text*	pInstance = new CUI_StageEnd_RS_Text(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CUI_StageEnd_RS_Text"));
...
CGameObject * CUI_StageEnd_RS_Text::Clone(void * pArg)
{
	CUI_StageEnd_RS_Text*	pInstance = new CUI_StageEnd_RS_Text(*this);
...
void CUI_StageEnd_RS_Text::Free()
{
	__super::Free();
}
