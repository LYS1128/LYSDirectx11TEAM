...
void CSky::Tick(_double TimeDelta)
{
	...
	if (CClient_Level_Manager::Get_Instance()->Get_Level() == CClient_Level_Manager::LEVEL_GAMEPLAY || CClient_Level_Manager::Get_Instance()->Get_Level() == CClient_Level_Manager::LEVEL_BOSS_FEAR)
		m_iTextureIndex = 1;
	else if(CClient_Level_Manager::Get_Instance()->Get_Level() == CClient_Level_Manager::LEVEL_CITY)
		m_iTextureIndex = 2;
	else if (CClient_Level_Manager::Get_Instance()->Get_Level() == CClient_Level_Manager::LEVEL_LAVA || CClient_Level_Manager::Get_Instance()->Get_Level() == CClient_Level_Manager::LEVEL_BOSS_RESENTMENT)
		m_iTextureIndex = 3;
	else
		m_iTextureIndex = 0;
}
...
HRESULT CSky::SetUp_Components()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::SetUp_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_SkyBox"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;	
...
HRESULT CSky::SetUp_ConstantTable()
{
	...
	if (FAILED(m_pTextureCom->SetUp_ShaderResourceView(m_pShaderCom, "g_DiffuseTexture", m_iTextureIndex)))
		return E_FAIL;
