#include "stdafx.h"
#include "..\Public\Dash_Distortion.h"
#include "GameInstance.h"

#include "Equipment_Manager.h"
#include "Time_Manager.h"

CDash_Distortion::CDash_Distortion(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{

}

CDash_Distortion::CDash_Distortion(const CDash_Distortion & rhs)
	: CGameObject(rhs)
{
}

HRESULT CDash_Distortion::NativeConstruct_Prototype()
{
...
HRESULT CDash_Distortion::NativeConstruct(void * pArg)
{
	...
	m_pTransformCom->Scaled(_float3(3.f*1.1f, 3.f*1.1f, 3.f*0.2f));
	m_fLength = 0.f;
...
void CDash_Distortion::Tick(_double TimeDelta)
{
...
void CDash_Distortion::LateTick(_double TimeDelta)
{
	_double TimeRatio = CTime_Manager::Get_Instance()->Get_PlayerRatio();
	_double mTimeDelta = TimeDelta * TimeRatio;
...
HRESULT CDash_Distortion::Render()
{
...
HRESULT CDash_Distortion::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CDash_Distortion::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CDash_Distortion * CDash_Distortion::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CDash_Distortion*	pInstance = new CDash_Distortion(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CDash_Distortion"));
...
CGameObject * CDash_Distortion::Clone(void * pArg)
{
	CDash_Distortion*	pInstance = new CDash_Distortion(*this);
