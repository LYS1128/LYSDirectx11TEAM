...
CMelee1_Sword::CMelee1_Sword(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CEnemy(pDeviceOut, pDeviceContextOut)
{

}

CMelee1_Sword::CMelee1_Sword(const CMelee1_Sword & rhs)
	: CEnemy(rhs)
{
}

HRESULT CMelee1_Sword::NativeConstruct_Prototype()
{
...
HRESULT CMelee1_Sword::NativeConstruct(void * pArg)
{
	...
	SocketDesc.pWP_TYPE = CMelee_Weapon::WP_TYPE::SWORD;

	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	m_pWeapon = pGameInstance->Add_GameObjToLayer(CClient_Level_Manager::Get_Instance()->Get_Level(), L"Layer_Sword", TEXT("Prototype_GameObject_Melee_Weapon"), &SocketDesc);
	RELEASE_INSTANCE(CGameInstance);	

	m_pEnemyModelCom->Set_PreAnimIndex(SPAWN);
	m_pEnemyModelCom->Set_AnimationIndex(SPAWN);
	m_iAnimationIndex = SPAWN;
	Info.eMonsterType = MSW_T1;
	Info._iHp = 20;
	Info._AttDmg = 5;
...
void CMelee1_Sword::Tick(_double TimeDelta)
{
	...
	if (bFirstSpawn)	// 플레이어랑 같은 레벨일때로 예외처리 할라했는데, 위에서 걍 리턴 때림
	{
		...
		if (m_pEnemyModelCom->Get_Animation(SPAWN)->Get_KeyFrameENd()) {
			bHavior = true;
			bFirstSpawn = false;
			Info.m_iRandomSword = Get_Randomint(1, 5);
		}
	}
	...
	if (bHavior) {
		...
		MotionControl();
		Delay(mTimeDelta, 1.5, 1, 5);
		Damaged_Red(mTimeDelta);
		if (!bCol)
			Searching_Player(mTimeDelta, 5.f, RUN_FORWARD, IDLE, Info.m_iRandomSword);
	}
...
void CMelee1_Sword::LateTick(_double TimeDelta)
{
	_double TimeRatio = CTime_Manager::Get_Instance()->Get_MonsterRatio();
	_double mTimeDelta = TimeDelta * TimeRatio;
	__super::LateTick(mTimeDelta);
	m_pEnemyModelCom->Set_Speed(1.3f);
...
HRESULT CMelee1_Sword::Render_LightDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pEnemyModelCom)
		return E_FAIL;
...
void CMelee1_Sword::Colliding_Bundle(_double TimeDelta)
{
	if (!bSuperArmor) {
...
void CMelee1_Sword::MotionControl()
{
	if (m_pEnemyModelCom->Get_Animation(m_iAnimationIndex)->Get_KeyFrameENd()) {
		if (0 < m_iAnimationIndex && m_iAnimationIndex < 6) {
			bDelay = true;
			bCol = false;
		}
		else {
			...
			bSuperArmor = false;
			bLookChase = true;
			Info.m_iRandomSpear = Get_Randomint(1, 5);
			m_pEnemyModelCom->Set_Speed(1.0f);
		}
	}
}

void CMelee1_Sword::Collision_Handling(EQUIP_INDEX _EquipmentIndex, _uint _AnimIndex1, _uint _AnimIndex2, _uint _AnimIndex3, _uint _AnimIndex4)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CMelee1_Sword::AnimTransform(_double TimeDelta)
{
	_bool	bFlag = false;
	...
	case SPEAR_ATTACK_LINE:
		...
		if (m_pEnemyModelCom->Get_Animation(SPEAR_ATTACK_LINE)->Get_TimeAcc() >= 25 && m_pEnemyModelCom->Get_Animation(SPEAR_ATTACK_LINE)->Get_TimeAcc() < 29) {
			m_pTransformCom->Go_StraightOnNavigation(TimeDelta * 10.0, &m_iCurrentCellIndex);
			bAtt = true;
		}
		else
			bAtt = false;
	...
	case SPEAR_ATTACK_THRUST:
		...
		if (m_pEnemyModelCom->Get_Animation(SPEAR_ATTACK_THRUST)->Get_TimeAcc() >= 29.0 && m_pEnemyModelCom->Get_Animation(SPEAR_ATTACK_THRUST)->Get_TimeAcc() < 50) {
			m_pTransformCom->Go_StraightOnNavigation(TimeDelta * 8.0, &m_iCurrentCellIndex);
			bAtt = true;
		}
		else
			bAtt = false;
...
HRESULT CMelee1_Sword::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CMelee1_Sword::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CMelee1_Sword * CMelee1_Sword::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CMelee1_Sword*	pInstance = new CMelee1_Sword(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CMelee1_Sword"));
...
CGameObject * CMelee1_Sword::Clone(void * pArg)
{
	CMelee1_Sword*	pInstance = new CMelee1_Sword(*this);
