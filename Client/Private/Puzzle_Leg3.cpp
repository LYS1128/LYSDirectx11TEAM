...
CPuzzle_Leg3::CPuzzle_Leg3(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{

}

CPuzzle_Leg3::CPuzzle_Leg3(const CPuzzle_Leg3 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CPuzzle_Leg3::NativeConstruct_Prototype()
{
...
HRESULT CPuzzle_Leg3::NativeConstruct(void * pArg)
{
...
void CPuzzle_Leg3::Tick(_double TimeDelta)
{
	...
	if (false == CPuzzle_Manager::Get_Instance()->Get_AllTrue() && true == CPuzzle_Manager::Get_Instance()->Get_bPlayerPos())
	{
		...
		{
		case 3:
			CPuzzle_Manager::Get_Instance()->Check_Pipe03(m_Level);
			break;
		case 18:
			CPuzzle_Manager::Get_Instance()->Check_Pipe18(m_Level);
			break;
		case 24:
			CPuzzle_Manager::Get_Instance()->Check_Pipe24(m_Level);
			break;
		...
		m_pAABBCom[2]->Update(XMMatrixTranslation(0.f, 0.f, 2.f)*m_pTransformCom->Get_WorldMatrix());
		m_pSphereCom->Update(m_pTransformCom->Get_WorldMatrix());
	}
}

void CPuzzle_Leg3::LateTick(_double TimeDelta)
{
	_double TimeRatio = CTime_Manager::Get_Instance()->Get_EctRatio();
	_double mTimeDelta = TimeDelta * TimeRatio;
	...
	if (false == CPuzzle_Manager::Get_Instance()->Get_AllTrue() && true == CPuzzle_Manager::Get_Instance()->Get_bPlayerPos())
	{
		...
		{
		case 3:
			m_bPipe = CPuzzle_Manager::Get_Instance()->Get_TruePipe(3);
			break;
		case 18:
			m_bPipe = CPuzzle_Manager::Get_Instance()->Get_TruePipe(18);
			break;
		case 24:
			m_bPipe = CPuzzle_Manager::Get_Instance()->Get_TruePipe(24);
			break;
...
HRESULT CPuzzle_Leg3::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::SetUp_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Prototype_Component_Model_Puzzle_Leg3"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
	...
	if (FAILED(__super::SetUp_Components(TEXT("Com_AABB2"), LEVEL_STATIC, TEXT("Prototype_Component_Collider_AABB"), (CComponent**)&m_pAABBCom[2], &ColliderDesc)))
		return E_FAIL;
...
HRESULT CPuzzle_Leg3::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CPuzzle_Leg3 * CPuzzle_Leg3::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CPuzzle_Leg3*	pInstance = new CPuzzle_Leg3(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CPuzzle_Leg3"));
...
CGameObject * CPuzzle_Leg3::Clone(void * pArg)
{
	CPuzzle_Leg3*	pInstance = new CPuzzle_Leg3(*this);
